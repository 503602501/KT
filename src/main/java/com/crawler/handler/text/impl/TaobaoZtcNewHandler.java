 ...
 * 淘宝直通车，业务点击处理
 */
public class TaobaoZtcNewHandler implements IText {
	...
	public String getText(String text,WebElement web,WebDriver webDriver,TextUnit textUnit,Storage storage) {
		...
		try {
			...
			//先清理红色和蓝色的标签
			focusClick("//table[@class='bp-table freeze-th']/thead/th[1]", webDriver);
			...
			Thread.sleep(2000);
			//点击昨天的日期
			webDriver.findElement(By.xpath("//span[@class='time-string']")).click();
			Thread.sleep(1500);
			...
			Thread.sleep(1500);
			webDriver.findElement(By.xpath("//div[@class='thead-title']/div/ul/li/span[contains(text(),'优化')]")).click();
			Thread.sleep(1500);
			//过滤出红色的标签 end end 
...
//					continue;
	...
	/*		//过滤出红色的标签
		...
		/*	webDriver.findElement(By.xpath("//table/thead/tr/th[3]/div/div/span")).click(); 
			...
			//平均排名排序，点击两次
			WebElement ee = webDriver.findElement(By.xpath("//div[contains(text(),'平均展现排名')]/i[1]"));  // 是否需要点击两次？？？？？排序有问题么？
			ee.click();
			Thread.sleep(2000);
			
			ee = webDriver.findElement(By.xpath("//div[@class='adgroup-detail']"));
...
//					webDriver.findElement(By.xpath("//div[@class='table-container']/table/tbody/tr[@id]["+(i+1)+"]/td[1]/input")).click();
			...
			if(ss.size()==0){
				return "没有符合平均展现排名关键词，所以没有做升价！！！->"+webDriver.getCurrentUrl();
			}
			selectList(webDriver, ss);
